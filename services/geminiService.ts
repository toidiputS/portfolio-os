...
export const speakText = async (text: string): Promise<string> => {
    try {
        ...
        // FIX: Updated payload to match correct text-to-speech API format and removed incorrect tool usage.
        const response = await generateContent({
        ...
        // FIX: Use a type guard to safely access the 'inlineData' property.
        const audioPart = response.candidates?.[0]?.content?.parts?.find(
            // FIX: The type predicate must match the `GeminiPart` union member, which includes `mimeType`.
            (p): p is { inlineData: { mimeType: string; data: string } } => 'inlineData' in p
        );
        const base64Audio = audioPart?.inlineData?.data;
        console.log('Generated audio data length:', base64Audio?.length || 0);
        return base64Audio || '';
    } catch (error: any) {
          ...
          try {
              ...
              utterance.voice = speechSynthesis.getVoices().find(voice => voice.name.includes('Zira') || voice.name.includes('Female')) || null;
              utterance.rate = 0.9;
              utterance.pitch = 1.1;
              speechSynthesis.speak(utterance);
              return 'web-speech-used';
