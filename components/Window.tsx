...
const Window: React.FC<WindowInstance & { children: React.ReactNode }> = ({
  id, appId, title, position, size, zIndex, minimized, snapState, preSnapSize, children
}) => {
  ...
  const handleDrag = (e: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (isSnapped) {
      ...
      updateWindowPosition(id, {
        ...
        updateWindowPosition(id, { x: position.x + info.delta.x, y: position.y + info.delta.y });
    }
  };
  ...
  const handleResize = (
    ...
    corner: string
      ...
      if (corner.includes('right')) newWidth = Math.max(minWidth, size.width + info.delta.x);
      if (corner.includes('bottom')) newHeight = Math.max(minHeight, size.height + info.delta.y);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center h-full text-[hsl(var(--foreground-hsl))]">
          <button type="button" onClick={() => minimizeWindow(id)} className="px-3 h-full hover:bg-[hsl(var(--secondary-hsl))] transition-colors" aria-label="Minimize window"><Minus size={16} /></button>
          <button type="button" onClick={handleMaximizeToggle} className="px-3 h-full hover:bg-[hsl(var(--secondary-hsl))] transition-colors" aria-label={snapState === 'maximized' ? "Restore window" : "Maximize window"}>
            {snapState === 'maximized' ? <Copy size={14} /> : <Square size={14} />}
          </button>
          <button type="button" onClick={() => closeWindow(id)} className="px-3 h-full hover:bg-[hsl(var(--destructive-hsl))] hover:text-[hsl(var(--destructive-foreground-hsl))] transition-colors" aria-label="Close window"><X size={16} /></button>
        </div>
      </motion.header>
      ...
      {!isSnapped && (
        <>
            <motion.div onPan={(e, info) => handleResize(info, 'left')} className="absolute -left-1 top-0 bottom-0 w-2 cursor-ew-resize" />
            <motion.div onPan={(e, info) => handleResize(info, 'right')} className="absolute -right-1 top-0 bottom-0 w-2 cursor-ew-resize" />
            <motion.div onPan={(e, info) => handleResize(info, 'top')} className="absolute -top-1 left-0 right-0 h-2 cursor-ns-resize" />
            <motion.div onPan={(e, info) => handleResize(info, 'bottom')} className="absolute -bottom-1 left-0 right-0 h-2 cursor-ns-resize" />
            <motion.div onPan={(e, info) => handleResize(info, 'topLeft')} className="absolute -top-1 -left-1 w-3 h-3 cursor-nwse-resize z-10" />
            <motion.div onPan={(e, info) => handleResize(info, 'topRight')} className="absolute -top-1 -right-1 w-3 h-3 cursor-nesw-resize z-10" />
            <motion.div onPan={(e, info) => handleResize(info, 'bottomLeft')} className="absolute -bottom-1 -left-1 w-3 h-3 cursor-nesw-resize z-10" />
            <motion.div onPan={(e, info) => handleResize(info, 'bottomRight')} className="absolute -bottom-1 -right-1 w-3 h-3 cursor-nwse-resize z-10" />
        </>
      )}
    </motion.div>
  );
